			...
			case 'challenge': {
				...
				try {
					...
					output += `<table style="width:100%; margin: 10px 0;"><tr>` +
						...
						`<span style="color: ${challengerColor};">${challengerRanking.rank}</span><br/>` +
						`<span style="color: ${battle.challengerEloChange >= 0 ? '#2ecc71' : '#e74c3c'};">${challengerRanking.elo} (${challengerEloChange})</span><br/>` +
						`<span style="color: #f39c12;">+${result.challengerCredits || 0} Credits</span>` +
						`</td>` +
						`<td style="width:50%; text-align:center; padding:10px;">` +
						`<strong>${targetUsername}</strong><br/>` +
						`Pack Value: <strong>${battle.targetPackValue}</strong> pts<br/>` +
						`<span style="color: ${targetColor};">${targetRanking.rank}</span><br/>` +
						`<span style="color: ${battle.targetEloChange >= 0 ? '#2ecc71' : '#e74c3c'};">${targetRanking.elo} (${targetEloChange})</span><br/>` +
					...
					output += `<hr/>` +
						`<table style="width:100%;"><tr>` +
						`<td style="width:50%; vertical-align:top; padding-right:5px;">` +
						`<strong>${Impulse.nameColor(user.name, true)}'s Pack (Total: ${battle.challengerPackValue} Points)</strong>` +
						`<table class="themed-table"> ${buildPackHtml(challengerPack)} </table>` +
						`</td><td style="width:50%; vertical-align:top; padding-left:5px;">` +
						`<strong>${Impulse.nameColor(targetUsername, true)}'s Pack (Total: ${battle.targetPackValue} Points)</strong>` +
						`<table class="themed-table"> ${buildPackHtml(targetPack)} </table>` +
						`</td></tr></table><hr/>`;
